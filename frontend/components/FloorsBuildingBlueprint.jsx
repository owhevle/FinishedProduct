...
const getDefaultRoomsForFloor = (building, floor) => {
  ...
  'GROUND FLOOR': () => {
    ...
    const stairsHeight = 120;
    const crWidth = 70;
    const crHeight = 60;
    const gap = 20;
    const verticalHallwayWidth = 80;
    const rightStairsWidth = 120;
    const rightStairsHeight = 80;

    // Left side stairs
    rooms.push(createRoom('STAIRS_LEFT', 'STAIRS', 'Stairs', startX, startY, stairsWidth, stairsHeight, 'no_request', 0, 'stairs'));

    // Male CR next to stairs
    const crMaleX = startX + stairsWidth + gap;
    rooms.push(createRoom('CR_MALE', 'CR M', 'Male CR', crMaleX, startY, crWidth, crHeight, 'no_request', 0));
    ...
    const firstHallwayY = startY + stairsHeight + gap/2;
    rooms.push(createRoom('HALLWAY_TOP_GF', 'HALL', 'Hallway', hallwayStartX, firstHallwayY, hallwayWidth, hallwayHeight, 'no_request', 0, 'hallway'));

    // NB2 on the right side
    ...
    // Calculate original Female CR position (for hallway reference)
    const originalCrFemaleX = nb2X;
    
    // Female CR at bottom right - aligned with end of NB3
    const nb3EndX = nb2X + roomWidth; // Right edge of NB3
    const crFemaleX = nb3EndX - crWidth; // Align right edge with NB3's right edge
    const crFemaleY = nb3Y + roomHeight + gap;
    rooms.push(createRoom('CR_FEMALE_GF', 'CR F', 'Female CR', crFemaleX, crFemaleY, crWidth, crHeight, 'no_request', 0));

    // Right side stairs below female CR - aligned with Female CR
    const stairsRightX = crFemaleX + crWidth - rightStairsWidth; // Align right edge with Female CR's right edge
    const stairsRightY = crFemaleY + crHeight + gap;
    rooms.push(createRoom('STAIRS_RIGHT_GF', 'STAIRS', 'Stairs', stairsRightX, stairsRightY, rightStairsWidth, rightStairsHeight, 'no_request', 0, 'stairs'));

    // Vertical hallway - STAYS IN ORIGINAL POSITION (to the left of original Female CR position)
    const hallwayVerticalX = originalCrFemaleX - verticalHallwayWidth - gap;
    const hallwayVerticalY = firstHallwayY;
    const hallwayVerticalHeight = (stairsRightY + rightStairsHeight) - firstHallwayY;
    rooms.push(createRoom('HALLWAY_VERTICAL_GF', 'HALL', 'Hallway', hallwayVerticalX, hallwayVerticalY, verticalHallwayWidth, hallwayVerticalHeight, 'no_request', 0, 'hallway'));

    const canvasWidth = Math.max(
      nb2X + roomWidth + gap * 2,
      crFemaleX + crWidth + gap,
      stairsRightX + rightStairsWidth + gap,
      hallwayVerticalX + verticalHallwayWidth + gap
    );
    const canvasHeight = Math.max(
      firstHallwayY + hallwayHeight + gap * 2,
      stairsRightY + rightStairsHeight + gap * 2
    );
  ...
  '2ND FLOOR': () => {
    ...
    // Left side stairs
    rooms.push(createRoom('STAIRS_LEFT_2F', 'STAIRS', 'Stairs', startX, startY, stairsWidth, stairsHeight, 'no_request', 0, 'stairs'));
    ...
    // Calculate original Female CR position (for hallway reference)
    const originalCrFemaleX = nb5X;
    
    // Female CR at bottom right - aligned with end of NB6
    const nb6EndX = nb5X + roomWidth; // Right edge of NB6
    const crFemaleX = nb6EndX - crWidth; // Align right edge with NB6's right edge
    const crFemaleY = nb6Y + roomHeight + gap;
    rooms.push(createRoom('CR_FEMALE_2F', 'CR F', 'Female CR', crFemaleX, crFemaleY, crWidth, crHeight, 'no_request', 0));

    // Right side stairs below female CR - aligned with Female CR
    const stairsRightX = crFemaleX + crWidth - rightStairsWidth; // Align right edge with Female CR's right edge
    const stairsRightY = crFemaleY + crHeight + gap;
    rooms.push(createRoom('STAIRS_RIGHT_2F', 'STAIRS', 'Stairs', stairsRightX, stairsRightY, rightStairsWidth, rightStairsHeight, 'no_request', 0, 'stairs'));

    // Vertical hallway - STAYS IN ORIGINAL POSITION (to the left of original Female CR position)
    const hallwayVerticalX = originalCrFemaleX - verticalHallwayWidth - gap;
    const hallwayVerticalY = firstHallwayY;
    const hallwayVerticalHeight = (stairsRightY + rightStairsHeight) - firstHallwayY;
    rooms.push(createRoom('HALLWAY_VERTICAL_2F', 'HALL', 'Hallway', hallwayVerticalX, hallwayVerticalY, verticalHallwayWidth, hallwayVerticalHeight, 'no_request', 0, 'hallway'));
  ...
  '3RD FLOOR': () => {
    ...
    // Left side stairs
    rooms.push(createRoom('STAIRS_LEFT_3F', 'STAIRS', 'Stairs', startX, startY, stairsWidth, stairsHeight, 'no_request', 0, 'stairs'));
    ...
    // Calculate original Female CR position (for hallway reference)
    const originalCrFemaleX = nb8X;
    
    // Female CR at bottom right - aligned with end of NB9
    const nb9EndX = nb8X + roomWidth; // Right edge of NB9
    const crFemaleX = nb9EndX - crWidth; // Align right edge with NB9's right edge
    const crFemaleY = nb9Y + roomHeight + gap;
    rooms.push(createRoom('CR_FEMALE_3F', 'CR F', 'Female CR', crFemaleX, crFemaleY, crWidth, crHeight, 'no_request', 0));

    // Right side stairs below female CR - aligned with Female CR
    const stairsRightX = crFemaleX + crWidth - rightStairsWidth; // Align right edge with Female CR's right edge
    const stairsRightY = crFemaleY + crHeight + gap;
    rooms.push(createRoom('STAIRS_RIGHT_3F', 'STAIRS', 'Stairs', stairsRightX, stairsRightY, rightStairsWidth, rightStairsHeight, 'no_request', 0, 'stairs'));

    // Vertical hallway - STAYS IN ORIGINAL POSITION (to the left of original Female CR position)
    const hallwayVerticalX = originalCrFemaleX - verticalHallwayWidth - gap;
    const hallwayVerticalY = firstHallwayY;
    const hallwayVerticalHeight = (stairsRightY + rightStairsHeight) - firstHallwayY;
    rooms.push(createRoom('HALLWAY_VERTICAL_3F', 'HALL', 'Hallway', hallwayVerticalX, hallwayVerticalY, verticalHallwayWidth, hallwayVerticalHeight, 'no_request', 0, 'hallway'));
    ...
    'DFA BUILDING': {
      ...
      '2ND FLOOR': () => {
        ...
        const topRooms = [
          { id: 'STAIRS_TOP', number: 'STAIRS', name: 'Stairs', special: 'stairs', width: stairsWidth },
          { id: 'A12', number: 'A12', name: 'Room A12', width: aWidth },
          { id: 'A10', number: 'A10', name: 'Room A10', width: aWidth },
          { id: 'A8', number: 'A8', name: 'Room A8', width: aWidth },
          { id: 'A6', number: 'A6', name: 'Room A6', width: aWidth },
          { id: 'A4', number: 'A4', name: 'Room A4', width: aWidth },
          { id: 'A2', number: 'A2', name: 'Room A2', width: aWidth },
          { id: 'DLL_MUSIC_ARTS', number: 'DLL', name: 'DLL Music & Arts', width: dllWidth, offsetX: 50}
        ];
        ...
        const bottomRooms = [
          { id: 'SSC_OFFICE', number: 'SSC', name: 'SSC Office', width: officeWidth },
          { id: 'CR_FEMALE', number: 'CR F', name: 'Female CR', width: officeWidth, special: 'cr' },
          { id: 'CR_MALE', number: 'CR M', name: 'Male CR', width: officeWidth, special: 'cr' },
          { id: 'A11', number: 'A11', name: 'Room A11', width: aWidth },
          { id: 'A9', number: 'A9', name: 'Room A9', width: aWidth },
          { id: 'A7', number: 'A7', name: 'Room A7', width: aWidth },
          { id: 'A5', number: 'A5', name: 'Room A5', width: aWidth },
          { id: 'A3', number: 'A3', name: 'Room A3', width: aWidth },
          { id: 'A1', number: 'A1', name: 'Room A1', width: aWidth }
        ];
        ...
        const canvasHeight = bottomY + aHeight + startY + 50;
    ...
    'ANNEX': {
      'GROUND FLOOR': () => {
        ...
        rooms.push(createRoom('GUIDANCE', 'GUIDANCE', 'Guidance Office', startX, currentY, officeWidth, officeHeight, 'no_request', 0));
        rooms.push(createRoom('STAIRS', 'STAIRS', 'Stairs', startX + officeWidth + gap, currentY, stairsWidth, stairsHeight, 'no_request', 0, 'stairs'));
        
        currentY += officeHeight + gap;
        rooms.push(createRoom('CLINIC', 'CLINIC', 'Clinic', startX, currentY, officeWidth, officeHeight, 'no_request', 0));
        ...
        roomNumbers.forEach((num) => {
          rooms.push(createRoom(num, num, `Room ${num}`, currentX, roomsStartY, roomWidth, roomHeight, 'no_request', 0));
        ...
        rooms.push(createRoom('CR_FEMALE_GF', 'CR F', 'Female CR', crStartX, crVerticalCenter, crWidth, crHeight, 'no_request', 0));
        rooms.push(createRoom('CR_MALE_GF', 'CR M', 'Male CR', crStartX, crVerticalCenter + crHeight + gap, crWidth, crHeight, 'no_request', 0));
        ...
        rooms.push(createRoom('PROPERTY', 'PROPERTY', 'Property Office', propertyX, propertyY, officeWidth, officeHeight, 'no_request', 0));
        
        const bottomSectionY = propertyY + officeHeight + gap * 2;
        const financeEndX = startX + officeWidth * 3 + gap * 2;
        
        rooms.push(createRoom('LOBBY', 'LOBBY', 'Lobby', startX, bottomSectionY, financeEndX - startX, officeHeight, 'no_request', 0));
        
        const bottomOffices = [
          { id: 'REGISTRATION', name: 'Registration Office' },
          { id: 'PRESIDENTS_OFFICE', name: 'President\'s Office' },
        ...
        const canvasHeight = bottomRowY + officeHeight + startY + 50;
      ...
      '2ND FLOOR': () => {
        ...
        rooms.push(createRoom('219', '219', 'Room 219', startX, currentY, officeWidth, officeHeight, 'no_request', 0));
        rooms.push(createRoom('STAIRS_2F', 'STAIRS', 'Stairs', startX + officeWidth + gap, currentY, stairsWidth, stairsHeight, 'no_request', 0, 'stairs'));
        ...
        rooms.push(createRoom('CR_FEMALE_2F', 'CR F', 'Female CR', crStartX, crVerticalCenter, crWidth, crHeight, 'no_request', 0));
        rooms.push(createRoom('CR_MALE_2F', 'CR M', 'Male CR', crStartX, crVerticalCenter + crHeight + gap, crWidth, crHeight, 'no_request', 0));
        ...
        rooms.push(createRoom('AVR_ROOM', 'AVR', 'AVR Room', avrX, avrY, avrWidth, avrHeight, 'no_request', 0));
        ...
        const canvasHeight = bottomRowY + roomHeight + startY + 50;
      ...
      '3RD FLOOR': () => {
        ...
        rooms.push(createRoom('SPEECH_LAB', 'SPEECH LAB', 'Speech Laboratory', startX, startY, speechLabWidth, speechLabHeight, 'no_request', 0));
        rooms.push(createRoom('STAIRS_3F', 'STAIRS', 'Stairs', startX + speechLabWidth + gap, startY, stairsWidth, stairsHeight, 'no_request', 0, 'stairs'));

        const comLab3X = startX + speechLabWidth + stairsWidth + gap * 2;
        rooms.push(createRoom('COMLAB_3', 'COMLAB 3', 'Computer Lab 3', comLab3X, startY, comLabWidth, comLabHeight, 'no_request', 0));

        const comLab4X = comLab3X + comLabWidth + gap;
        rooms.push(createRoom('COMLAB_5', 'COMLAB 5', 'Computer Lab 5', comLab4X, startY, comLabWidth, comLabHeight, 'no_request', 0));
        ...
        const bottomRowY = hallwayY + hallwayHeight + gap;
        rooms.push(createRoom('COMLAB_1', 'COMLAB 1', 'Computer Lab 1', startX, bottomRowY, comLabWidth, comLabHeight, 'no_request', 0));

        const comLab2X = startX + comLabWidth + gap;
        rooms.push(createRoom('COMLAB_2', 'COMLAB 2', 'Computer Lab 2', comLab2X, bottomRowY, comLabWidth, comLabHeight, 'no_request', 0));

        rooms.push(createRoom('COMLAB_4', 'COMLAB 4', 'Computer Lab 4', comLab4X, bottomRowY, comLabWidth, comLabHeight, 'no_request', 0));
        ...
        const canvasHeight = maxBottom + startY + 50;
      ...
      '4TH FLOOR': () => {
        ...
        const totalHeight = startY + officeHeight + gap + room401Height + 90;
...
const FloorsBuildingBlueprint = () => {
  ...
  // Function to render the blueprint header
  const renderBlueprintHeader = (canvasWidth, canvasHeight) => {
    const headerText = `${currentBuilding} - ${currentFloor}`;
...
const fetchRooms = useCallback(async () => {
  ...
  try {
    ...
    if (response.data && response.data.length > 0) {
      ...
      // Create a map of API rooms for easier lookup
      const apiRoomsMap = {};
      response.data.forEach(apiRoom => {
        ...
        const identifiers = [
          ...
          apiRoom.room_id,
          apiRoom.room_number,
          apiRoom.name?.toLowerCase()
        ].filter(Boolean);
        
        identifiers.forEach(id => {
          if (id && !apiRoomsMap[id]) {
            apiRoomsMap[id] = apiRoom;
          }
        });
      });
      
      const updatedRooms = blueprint.rooms.map(room => {
        // Try to find matching API room
        let apiRoom = null;
        
        // Try multiple matching strategies
        if (room.id && apiRoomsMap[room.id]) {
          apiRoom = apiRoomsMap[room.id];
        } else if (room.room_number && apiRoomsMap[room.room_number]) {
          apiRoom = apiRoomsMap[room.room_number];
        } else if (room.room_name && apiRoomsMap[room.room_name.toLowerCase()]) {
          apiRoom = apiRoomsMap[room.room_name.toLowerCase()];
        } else {
          // Try to find by partial match
          for (const apiRoomItem of response.data) {
            if (
              apiRoomItem.room_number === room.room_number ||
              apiRoomItem.name?.toLowerCase() === room.room_name?.toLowerCase() ||
              (apiRoomItem.building === currentBuilding && 
               apiRoomItem.floor === currentFloor &&
               apiRoomItem.room_number?.includes(room.room_number?.replace(/[^A-Z0-9]/gi, '')))
            ) {
              apiRoom = apiRoomItem;
              break;
            }
        ...
        // If API has this room, ONLY update status and request_count
        if (apiRoom) {
          return {
            ...room, // Keep all blueprint properties
            id: apiRoom.id || room.id,
            room_number: apiRoom.room_number || room.room_number,
            room_name: apiRoom.room_name || apiRoom.name || room.room_name,
            building: apiRoom.building || room.building,
            floor: apiRoom.floor || room.floor,
            status: apiRoom.status || room.status,
            request_count: apiRoom.request_count || apiRoom.requests_count || 0,
            special: room.special // Always keep the blueprint's special designation
          };
        }
        
        // If no API match, keep the blueprint room as-is
        return room;
      });
      ...
      // No API data, use blueprint as-is
      const blueprint = getDefaultRoomsForFloor(currentBuilding, currentFloor);
  ...
  const renderBlueprint = () => {
    ...
    const viewHeight = Math.min(700, canvasHeight + 60);
    
    const adjustedCanvasHeight = canvasHeight + 70;
    ...
    return (
      <div className="relative">
        <svg 
          width={viewWidth} 
          height={viewHeight} 
          viewBox={`0 0 ${canvasWidth} ${adjustedCanvasHeight}`}
        ...
        >
          {createBlueprintPaper(canvasWidth, adjustedCanvasHeight)}
          ...
          {renderBlueprintHeader(canvasWidth, adjustedCanvasHeight)}
          
          {showGrid && createBlueprintGrid(canvasWidth, adjustedCanvasHeight)}
          ...
          {filteredRooms.map(room => {
            ...
            return renderRoom(adjustedRoom);
          })}
          
          {currentBuilding === 'DFA BUILDING' && currentFloor === '2ND FLOOR' && (
            <g transform={`translate(${canvasWidth - 150}, ${adjustedCanvasHeight - 30})`}>
        ...
        {(statusFilter !== 'all' || searchTerm.trim()) && (
          <div className="absolute top-16 left-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white px-3 py-1 rounded-full text-sm shadow-lg">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm">floor plan visualization</p>
          </div>
        </div>
        
        <div className="flex items-center space-x-4">
          <button className="p-2 rounded-lg bg-white hover:bg-gray-50 transition-colors shadow-sm">
